...
for i in range(100, len(df)):
    ...
    fee_cost = 0
    if prev_weights:
        change = sum(abs(weights.get(k,0)-prev_weights.get(k,0)) for k in set(weights)|set(prev_weights))
        fee_cost = change * trade_fee_rate * equity[-1]
    prev_weights = weights

    # ===== 槓桿調整 =====
    d_lev = max_lev
    if row_vix > 40: d_lev = min(d_lev, 0.5)
    elif row_vix > 30: d_lev = min(d_lev, 0.6)
    elif row_vix > 20: d_lev = min(d_lev, 0.8)

    # ===== 當日回報計算 =====
    day_ret = (returns.iloc[i][m1]*weights[m1] + returns.iloc[i][m2]*weights[m2]) * d_lev
    equity.append(equity[-1]*(1+day_ret) - fee_cost)
